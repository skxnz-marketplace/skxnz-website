...
alter table public.return_request_items add column if not exists reason text;
...
begin
 ...
 if exists(select 1 from jsonb_array_elements(p_items) j(v) group by v->>'order_item_id' having count(*)>1) then return jsonb_build_object('ok',false,'code','VALIDATION_FAILED'); end if;
 ...
 for v in select (x->>'order_item_id')::uuid item_id,(x->>'quantity')::int qty from jsonb_array_elements(p_items) j(x) order by (x->>'order_item_id')::uuid loop
